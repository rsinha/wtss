...
impl<C: CurveGroup + Hash> ThresholdSchnorr<C> where C::ScalarField: PrimeField,
{
    ...
    pub fn sign_round3(
        ...
        let aggregate_pk = public_keys
            .iter()
            .fold(C::Affine::zero(), |acc, pk| (acc + pk).into_affine());
...
mod tests {
    ...
    fn thresholdschnorr_sign_rounds_and_aggregate_should_succeed() {
        ...
        // Round 2
        let r2_msg1 = ThresholdSchnorr::<C>::sign_round2(&params).unwrap();
        let r2_msg2 = ThresholdSchnorr::<C>::sign_round2(&params).unwrap();
