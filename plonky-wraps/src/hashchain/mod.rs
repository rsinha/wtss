...
mod tests {
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        let tmp = builder.build::<C>();
        println!("vk digest: {:?}", tmp.verifier_only.circuit_digest);
    ...
    fn test_cyclic_recursion() -> Result<()> {
        ...
        println!("Inner cyclic proof with PIS: {:?}", inner_cyclic_proof_with_pis.public_inputs.len());
        ...
        let actual_hash_in = HashOutTarget {
            elements: core::array::from_fn(|i| builder.select(
                condition,
